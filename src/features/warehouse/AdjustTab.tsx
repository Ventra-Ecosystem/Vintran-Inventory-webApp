...
export function AdjustTab() {
  ...
  useEffect(() => {
    ...
    productsApi.list({ search: productSearch })
  ...
  const handleConfirm = async () => {
    if (!reason) { toast.error('Please select an adjustment reason'); return; }
    setSubmitting(true);
    try {
      if (selectedProduct?.id && selectedLoc?.id) {
      ...
      toast.error(err instanceof ApiError ? err.description : 'Failed to save adjustment');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const variance = quantity - (selectedProduct?.stock ?? selectedProduct?.totalQuantity ?? 0);
    ...
    <>
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              {historyItems.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">No adjustments yet</p>
              ) : (
                    ...
                    <div key={item.id} className="flex items-center gap-3 py-3 border-b border-gray-100 last:border-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-gray-900 truncate">{item.productName ?? item.product}</p>
                        </div>
                        <p className="text-xs text-gray-500 mt-0.5">{item.reason ?? item.movementType}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className={cn("text-sm font-semibold", (item.quantityDelta ?? item.quantity ?? 0) >= 0 ? "text-green-600" : "text-red-500")}>
                          {(item.quantityDelta ?? item.quantity ?? 0) >= 0 ? '+' : ''}{item.quantityDelta ?? item.quantity ?? 0} units
                        </p>
        ...
        {step === 'location' && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {loadingLocations ? (
                <p className="text-sm text-gray-500 text-center py-4">Loading locations…</p>
              ) : locations.length === 0 ? (
        ...
        {step === 'product' && (
          <div className="space-y-6">
            ...
            <div className="space-y-0">
              ...
              ) : products.filter((p: any) => (p?.name ?? '').toLowerCase().includes(productSearch.toLowerCase()) || (p?.sku ?? '').toLowerCase().includes(productSearch.toLowerCase())).map((product: any) => (
                <button
                  key={product.id}
                  onClick={() => {
                    setSelectedProduct(product);
                    setQuantity(product.stock ?? product.totalQuantity ?? 0);
                    setStep('form');
                  }}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="h-12 px-3.5 rounded-xl border border-gray-200 bg-gray-50 flex items-center">
                <span className="text-sm font-medium text-gray-500">{selectedProduct?.stock ?? selectedProduct?.totalQuantity ?? 0}</span>
              </div>
            </div>
            ...
            {quantity !== (selectedProduct?.stock ?? selectedProduct?.totalQuantity ?? 0) && (
              <div className="flex items-center gap-2">
      ...
      <Modal isOpen={showSuccess} onClose={handleDone}>
        <SuccessScreen
          ...
          subtitle={`Inventory count updated for ${selectedProduct?.name || 'product'}.`}
          details={[
            { label: 'Product', value: selectedProduct?.name || '—' },
            { label: 'New Quantity', value: String(quantity) },
            { label: 'Reason', value: reason || '—' },
            { label: 'Location', value: selectedLoc?.name || '—' },
          ]}
          primaryAction={
