...
export function AlertsTab({ onViewProduct }: AlertsTabProps) {
  ...
  useEffect(() => {
    reportsApi.inventory()
      ...
      .catch((err: unknown) => toast.error(err instanceof ApiError ? err.description : 'Failed to load alerts'))
  ...
  return (
    <div className="space-y-4">
      <SegmentedTabs
        options={[{ value: 'low-stock', label: 'Low stock' }, { value: 'out-of-stock', label: 'Out of stock' }]}
        value={filter}
        onChange={setFilter}
      />
      {loading ? (
        ...
        <div className="text-center py-8 text-text-muted text-sm">No alerts</div>
      ) : displayed.map((item) => {
        const pct = item.lowStockThreshold > 0 ? Math.min((item.totalQuantity / item.lowStockThreshold) * 100, 100) : 0;
        return (
          <div key={item.productId} className="rounded-[12px] px-3 py-2.5 bg-bg-surface flex flex-col gap-4">
            <div className="flex gap-2 items-center">
              <PakageIcon width={24} />
              <div>
                <p className="text-text-subtle font-semibold text-sm">{item.productName}</p>
                <p className="text-text-subtle text-xs font-medium">{item.sku}</p>
              </div>
            </div>
